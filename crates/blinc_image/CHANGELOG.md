...
### Added
- **Pluggable [`ImageDecoder`] trait + [`DecoderRegistry`]** for BYO format support. Apps can register a custom decoder (zune-image, libvips, a WebGPU compute decoder, a stub for headless tests) via [`set_global_registry`]; the loader picks it up. Built-in decoders ship as [`PngDecoder`], [`JpegDecoder`], etc., each gated by its matching cargo feature.
...
[`set_global_registry`]: crate::decoder::set_global_registry
[`PngDecoder`]: crate::decoder
[`JpegDecoder`]: crate::decoder
