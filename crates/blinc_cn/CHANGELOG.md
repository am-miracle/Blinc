...
### Added
- **`cn::number_input`** — themed wrapper around `blinc_layout::widgets::number_input` with a `+` / `−` stepper pair flanking the field. Reuses `InputSize` from `cn::input` for height / font alignment (so a number input in a form row sits flush with text inputs of the matching size). Stepper buttons read the bound `State<f64>`, clamp against `min` / `max`, and write back via `state.set`. All parse / clamp / format work lives in the layout widget; cn contributes only the surface + the stepper affordance per the layering rule.
