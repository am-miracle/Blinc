...
impl FontRegistry {
    ...
    fn load_emoji_font(&mut self) -> Result<Arc<FontFace>> {
        ...
        // Bundled NotoColorEmoji subset fallback.
        //
        // On wasm there are zero system fonts, and on
        // Linux/Android/Windows the user's system may not have a
        // color-emoji font installed. We ship a pre-subsetted
        // NotoColorEmoji (CBDT/CBLC) as `include_bytes!` — the
        // subset is generated by `scripts/regen-emoji-subset.sh`
        // from the codepoints that `blinc-emoji-scan` harvests from
        // the workspace, so it stays small (~150 KB vs the ~10 MB
        // upstream).
        //
        // Apple targets always have Apple Color Emoji installed at
        // a well-known system path, so we skip the bundled fallback
        // there to avoid the binary-size cost.
        if let Some(face) = self.load_bundled_emoji_font(&cache_key) {
            return Ok(face);
        }
    ...
    /// Load the bundled NotoColorEmoji subset. Returns `None` on
    /// Apple targets (system font is always present) or if the bytes
    /// fail to parse.
    #[cfg(not(any(target_os = "macos", target_os = "ios")))]
    fn load_bundled_emoji_font(&mut self, cache_key: &str) -> Option<Arc<FontFace>> {
        // ~150 KB subset built by `scripts/regen-emoji-subset.sh`.
        // The file is committed to the repo so fresh checkouts
        // don't need harfbuzz installed to build.
        const BUNDLED_EMOJI: &[u8] = include_bytes!("../assets/fonts/NotoColorEmoji-subset.ttf");

        let loaded = self.load_font_data(BUNDLED_EMOJI.to_vec());
        if loaded == 0 {
            tracing::warn!("Bundled NotoColorEmoji subset failed to parse");
            return None;
        }

        // `load_font_data` registers faces into fontdb but doesn't
        // wire them up to the `self.faces` cache. Look the face up
        // by family name so the rest of the shaper flow finds it.
        let face = self.load_font("Noto Color Emoji").ok()?;
        self.faces
            .insert(cache_key.to_string(), Some(Arc::clone(&face)));
        tracing::debug!("Loaded bundled NotoColorEmoji subset fallback");
        Some(face)
    }

    /// Apple targets always have Apple Color Emoji — no need for a
    /// bundled fallback.
    #[cfg(any(target_os = "macos", target_os = "ios"))]
    fn load_bundled_emoji_font(&mut self, _cache_key: &str) -> Option<Arc<FontFace>> {
        None
    }
